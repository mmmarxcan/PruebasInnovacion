...
export function RegisterScreen({ onRegister, onBack }: RegisterScreenProps) {
  ...
  return (
    <div className="h-full flex flex-col bg-gradient-to-b from-cyan-50/50 via-white to-purple-50/50">
      ...
      <div className="p-4 flex items-center animate-in fade-in duration-300">
        ...
        <div className="flex-1 text-center pr-10">
          <span className="text-sm text-slate-600 font-medium">Crear cuenta</span>
        </div>
      </div>

      {/* Contenido principal */}
      <div className="flex-1 flex flex-col px-8 pb-8 overflow-y-auto">
        <div className="mb-6 text-center animate-in fade-in slide-in-from-top duration-500">
          ...
          <p className="text-slate-600">Crea tu cuenta gratis</p>
        </div>

        <form onSubmit={handleRegister} className="space-y-4 animate-in fade-in duration-700">
          <div>
            <label className="text-sm text-slate-700 mb-2 block font-medium">Nombre completo</label>
          ...
          <div>
            <label className="text-sm text-slate-700 mb-2 block font-medium">Confirmar contraseña</label>
            <div className="relative">
              <Lock className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-400" />
              <Input
                type="password"
